...
Use this endpoint to **discover every metric available for a single project/chain** by applying the `project` query parameter. The response includes the metric name, description, stable `identifier` (used with [Get Single Metric](/api-reference/metrics/get)), data type, and data lineage source.
