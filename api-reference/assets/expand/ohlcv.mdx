...
description: "Retrieve ohlcv_last_24h data for an asset using the expand parameter"
...
Add the `?expand=ohlcv_last_24h` flag to your `/assets` or `/assets/{idOrSlug}` page to include **ohlcv_last_24h** data in the response.
...
curl -H "x-api-key: YOUR_API_KEY" \
  "https://api.blockworks.com/v1/assets/ethereum?expand=ohlcv_last_24h"
```
...
console.log(data.ohlcv_last_24h)
```
...
r = requests.get(
  ...
  headers={'x-api-key': 'YOUR_API_KEY'},
  params={'expand': 'ohlcv_last_24h'}
)
data = r.json()
print(data.get('ohlcv_last_24h'))
```
