...
export const syncMiscMetrics = async () => {
  ...
  console.log(c.subHeader('\n  1. Generating market-stats page...'))
  ...
  try {
    ...
    console.log(c.adding('     + market-stats.mdx'))
    ...
    console.log(c.green('\n  ✅ Misc endpoints updated successfully'))
    ...
    console.error(c.error('  ❌ Error generating misc metrics:'), error)
    throw error
  }
}
...
export const updateMiscNavigation = async (docsPath: string = './docs.json') => {
  ...
  if (!apiTab) {
    console.error(c.error('API Documentation tab not found in docs.json'))
    return
  }
  ...
  console.log(c.green('  ✅ Updated docs.json navigation for misc metrics'))
}